...
class Workout {
  ...
  createID() {
    return String(Math.trunc(Math.random() * 100) + Date.now()).slice(-10);
  }
...
class App {
  ...
  _loadMap(position) {
    ...
    // Clicking on map event
    this.#map.on('click', this._showForm.bind(this));

    // Reload data from local storage on map
  ...
  _newWorkout(e) {
    ...
    if (type === 'cycling') {
      const elevation = +inputElevation.value.trim();
  ...
  _moveToPopup(e) {
    ...
    this.#map.setView(workoutMarker.coords, this.#mapZoomLevel, {
      animate: true,
      pan: {
        duration: 1,
      },
